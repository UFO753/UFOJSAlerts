...
function UFOAlertInputValue() {
  const input_value = document.getElementById("UFOJSInputField").value;
...
let alert, AlertTextTop, AlertTextBottom;
...
function UFOAlert(alert_title, text, button_text) {
  CreateElem();
  const button = createButton(button_text, "UFOAlertClose()", "UFOAlertButton");
  console.log(button);
  ...
  setTimeout(function () {
    if (document.getElementById("UFOAlert") != null) {
      document.getElementById("UFOAlert").style.top = "10px";
    }
  }, 300);
}
...
function UFOAlertConfirm(alert_title, text, button_false, button_true) {
  CreateElem();
  const button_false = createButton(
    button_false,
    "UFOAlertFalse()",
    "UFOAlertFalseButton"
  );
  const button_true = createButton(
    button_true,
    "UFOAlertTrue()",
    "UFOAlertTrueButton"
  );
...
function UFOAlertInput(alert_title, button_text) {
  ...
  alert.innerText = "";
  AlertTextTop.innerText = alert_title;
  const button_confirm = createButton(
    button_text,
    "UFOAlertInputValue()",
    "UFOAlertButton"
  );
